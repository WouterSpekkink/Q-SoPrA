...
bool RelationshipTreeFilter::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
  ...
  QStandardItem *currentItem = currentModel->
    itemFromIndex(sourceModel()->index(sourceRow, 0, sourceParent));
  return (!currentItem->parent() || (currentItem->
				     data(Qt::DisplayRole).toString().contains(filterRegularExpression())));
}
